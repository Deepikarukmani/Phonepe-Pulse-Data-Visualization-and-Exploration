...
st.set_page_config( page_title="Phonepe pulse Data Visualization - By M.Deepika", page_icon=img,layout="wide",initial_sidebar_state="expanded", menu_items={"About": """# This dashboard app is crated by M.Deepika"""})
...
if Select=="Home":
    image1=Image.open("C:/Users/deepi/Downloads/pulse-master/pulse-master/PhonePe.jpg")
    ...
    with c2:
        image2=Image.open("C:/Users/deepi/Downloads/pulse-master/pulse-master/PhonePe.jpg")
        st.image(image2)
...
# Top Charts - USERS 
    ...
    if A=="Pincode":
        ...
        if B==2022 or (C==1 or C==2 or C==3):
            ...
            mycursor.execute(f"select Pincode,sum(Registered_users) as Registered_users from top_user where year={B} and quarter={C} group by Pincode order by Registered_users desc limit 10")
            ...
            fig = px.pie(df, title="Top 10 Registered_users", values="Registered_users", names="Pincode", color_discrete_map=color_mapping)
            fig.update_traces(textposition="inside", textinfo="percent+label")
            st.plotly_chart(fig,use_container_width=True)
